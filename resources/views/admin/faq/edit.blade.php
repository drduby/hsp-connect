...
@section('content')
    ...
    <div class="flex items-center justify-between mb-8">
        <h1 class="text-2xl font-bold text-gray-900">Edit FAQ Item</h1>
    </div>

    <div class="overflow-hidden rounded-xl bg-white shadow ring-1 ring-gray-200">
        <form method="POST" action="{{ route('admin.faq.update', $faq) }}" class="divide-y divide-gray-100">
            @csrf @method('PUT')
            ...
            <div class="px-6 py-6 grid grid-cols-1 gap-6 sm:grid-cols-2">
                <div>
                    ...
                    <textarea name="question" rows="3"
                              class="block w-full rounded-md border border-gray-300 px-3 py-2 text-sm shadow-sm focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500">{{ old('question', $faq->question) }}</textarea>
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Question (EN)</label>
                    <textarea name="question_en" rows="3"
                              class="block w-full rounded-md border border-gray-300 px-3 py-2 text-sm shadow-sm focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500">{{ old('question_en', $faq->question_en) }}</textarea>
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Answer (DE)</label>
                    <textarea name="answer" rows="5"
                              class="block w-full rounded-md border border-gray-300 px-3 py-2 text-sm shadow-sm focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500">{{ old('answer', $faq->answer) }}</textarea>
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Answer (EN)</label>
                    <textarea name="answer_en" rows="5"
                              class="block w-full rounded-md border border-gray-300 px-3 py-2 text-sm shadow-sm focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500">{{ old('answer_en', $faq->answer_en) }}</textarea>
                </div>
